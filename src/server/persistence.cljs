...
(->
 ...
 (.then
   (fn []
     (.then (.query client "select * from state")
            (fn [data]
              (let [data                 (aget (.-rows data) "0")
                ...
                (js/console.log "state %s %s %s" r t p) (reset! todos (map-todos t)) (reset! rooms (set (map keyword r))) (reset! passwords (to-keywords p)))))))
 (.catch (fn [err] (js/console.log "catch!" err))))

(defn watch [db-client atom column-name]
  (add-watch
   atom (keyword column-name)
   (fn [_ _ old-state new-state]
     (when (not= old-state new-state)
       (js/console.log (str column-name " new state: %s") new-state)
       (->
        (.query db-client (str "update state set " column-name " = $1::json where idx=1")
                (clj->js [(js/JSON.stringify (clj->js new-state))]))
        (.then (fn [x] (js/console.log "OK:" x)))
        (.catch (fn [x] (js/console.log "ERR:" x))))))))
